...
/**
 * Repository interface for managing DatingPool entities. Provides methods for querying and
 * retrieving potential matches based on user preferences and attributes. Implements custom queries
 * optimized for matching operations.
 */
public interface MatchingRepository extends JpaRepository<DatingPool, Long> {
   ...
   *
   * <p>This query ensures that: - The matched users' gender matches the requesting user's
   * preference (`lookingForGender`). - The matched users are looking for the requesting user's
   * gender (`gender`). - Both users' ages fall within each other's preferred age ranges. - The
   * matched users are located within the specified geohash prefixes (derived from the user's radius
   * preference). - The requesting user's location is within the matched users' acceptable geohash
   * areas.
   *
   * <p>The use of 3-character geohash prefixes via precision balances performance and accuracy by
   * pre-filtering users in broad geographic areas before precise distance calculations.
   *
