...
/**
 * Service responsible for synchronizing user profile data with the dating pool system. This service
 * ensures that the {@link DatingPool} entry for a user reflects the latest information from their
 * {@link UserProfile}, {@link UserAttributes}, {@link UserPreferences}, and {@link UserScore}. The
 * synchronization is triggered by changes to these entities and is essential for maintaining
 * accurate matching recommendations.
 */
@Service
...
public class DatingPoolSyncService {
   ...
   *
   * <p>This method performs the following steps: 1. Retrieves the user's {@link UserAttributes},
   * {@link UserPreferences}, and {@link UserProfile}. 2. Checks if all required fields are present
   * in these entities. If any required data is missing, the synchronization is skipped, and a debug
   * log is recorded. 3. Fetches or creates a {@link DatingPool} entry for the user. 4. Updates the
   * {@link DatingPool} entry with the latest user data, including gender, age, location (as a
   * geohash), hobby IDs, and matching preferences. 5. Saves the updated {@link DatingPool} entry to
   * the repository.
   *
   * <p>The synchronization is skipped if any of the following conditions are met: - {@link
   * UserAttributes} is missing or lacks required fields (gender, birthdate, location). - {@link
   * UserPreferences} is missing or lacks required fields (gender, ageMin, ageMax, distance). -
   * {@link UserProfile} is missing.
   *
   * <p>If the {@link UserScore} is missing when creating a new {@link DatingPool} entry, a {@link
   * ResourceNotFoundException} is thrown.
   *
   * @param profileId The unique identifier of the user profile to synchronize
   * @throws ResourceNotFoundException if the {@link UserScore} cannot be found when creating a new
   *     {@link DatingPool} entry
   */
  ...
  public void synchronizeDatingPool(Long profileId) {
    ...
    DatingPool entry =
        matchingRepository.findById(profileId).orElseGet(() -> createNewDatingPoolEntry(profileId));
   ...
   *
   * <p>This method is called when no existing {@link DatingPool} entry is found for the user. It
   * retrieves the user's {@link UserScore} and initializes a new {@link DatingPool} entry with the
   * profile ID and the current score.
   *
  ...
  private DatingPool createNewDatingPoolEntry(Long profileId) {
    UserScore userScore =
        userScoreRepository
            .findById(profileId)
            .orElseThrow(
                () -> new ResourceNotFoundException("UserScore for profile ID: " + profileId));
   ...
   * @return {@code true} if gender, ageMin, ageMax, and distance are present; {@code false}
   *     otherwise
   */
   ...
   */
  private void updateDatingPoolEntry(
      DatingPool entry,
