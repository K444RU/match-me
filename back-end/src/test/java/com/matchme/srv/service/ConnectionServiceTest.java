...
class ConnectionServiceTest {
  ...
  void setUp() {
    ...
    requester =
        TestDataFactory.createUser(TestDataFactory.DEFAULT_USER_ID, TestDataFactory.DEFAULT_EMAIL);
    target =
        TestDataFactory.createUser(
            TestDataFactory.DEFAULT_TARGET_USER_ID, TestDataFactory.DEFAULT_TARGET_EMAIL);
  ...
  void getUserConnections_success() {
    ConnectionState acceptedState =
        createState(ConnectionStatus.ACCEPTED, requester.getId(), target.getId());
    connection.getConnectionStates().add(acceptedState);
    when(connectionRepository.findConnectionsByUserId(requester.getId()))
    ...
    assertEquals(
        target.getId(),
        result.getActive().getFirst().getUserId(),
        "The active connection should be with the target user");
    assertEquals(
        connection.getId(),
        result.getActive().getFirst().getConnectionId(),
        "The connection ID should match");
    assertTrue(
        result.getPendingIncoming().isEmpty(), "There should be no pending incoming requests");
    assertTrue(
        result.getPendingOutgoing().isEmpty(), "There should be no pending outgoing requests");
  ...
  void sendConnectionRequest_success_newConnection() {
    ...
    when(connectionRepository.findConnectionBetween(requester.getId(), target.getId()))
        .thenReturn(null);
  ...
  void sendConnectionRequest_pendingExists_throwsIllegalStateException() {
    ConnectionState state =
        createState(ConnectionStatus.PENDING, requester.getId(), target.getId());
        ...
        assertThrows(
            ...
            () ->
                connectionService.acceptConnectionRequest(
                    DEFAULT_CONNECTION_ID, requester.getId()));
  ...
  void rejectConnectionRequest_success() {
    ...
    when(userRepository.findById(target.getId())).thenReturn(Optional.of(target));
    when(connectionRepository.save(any(Connection.class))).thenReturn(connection);
    doNothing().when(userScoreService).updateUserScore(target.getId(), requester.getId(), false);

    connectionService.rejectConnectionRequest(DEFAULT_CONNECTION_ID, target.getId());

    verify(connectionRepository).save(any(Connection.class));
    verify(userScoreService).updateUserScore(target.getId(), requester.getId(), false);
  }
  ...
  void getConnections_pendingIncoming() {
    ConnectionState pendingState =
        createState(ConnectionStatus.PENDING, target.getId(), requester.getId());
    connection.getConnectionStates().add(pendingState);
    when(connectionRepository.findConnectionsByUserId(requester.getId()))
        .thenReturn(List.of(connection));
